	...
	public async search(
		...
		if (params.metadata && typeof params.metadata === "object") {
			...
			for (const [keyRaw, valRaw] of Object.entries(params.metadata)) {
				...
				const left = Sequelize.literal(`("User"."metadata"->>'${key}')`);
		...
		if (params.accountMetadata && typeof params.accountMetadata === "object") {
			const accountWhere = {};
			accountWhere[Op.and] = accountWhere[Op.and] ?? [];

			for (const [keyRaw, valRaw] of Object.entries(params.accountMetadata)) {
				if (valRaw === null || valRaw === undefined) continue;
				...
				const left = Sequelize.literal(`("account"."metadata"->>'${key}')`);

				if (value.includes("*")) {
					accountWhere[Op.and].push(Sequelize.where(left, { [Op.iLike]: value.replace(/\*/g, "%") }));
				} else {
					accountWhere[Op.and].push(Sequelize.where(left, { [Op.eq]: value }));
				}
			}
